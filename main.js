...
const threeLine  = async(base , conv ,span_a ,span_b ,chiko) =>{
  ...
  if(conv[0] >= span_b[0] && conv[0]>=span_a[0])
  {
    for(let i = 1 ; i<7 ; i++)
    {
      let m = (conv[i] - span_a[i]) * (conv[i] - span_b[i])
      if(m<0)
      {
        //conv was in cloud
        flagConvBuy = true
  ...
  if(base[0] >= span_b[0] && base[0]>=span_a[0])
  {
    for(let i = 1 ; i<7 ; i++)
    {
      let m = (base[i] - span_a[i]) * (base[i] - span_b[i])
      if(m<0)
      {
        //base was in cloud
        flagBaseBuy = true
  ...
  if(chiko[26] >= span_b[26] && chiko[26]>=span_a[26])
  {
    for(let i = 27 ; i<34 ; i++)
    {
      let m = (chiko[i] - span_a[i]) * (chiko[i] - span_b[i])
      if(m<0)
      {
        //base was in cloud
        flagChikoBuy = true
...
while(1)
  {
    ...
    for(let i =0 ; i<coins.length ; i++)
    {
      let sag  = await ichimokuIndicator(symbol = coins[i] , interval = "1h")
      if(sag != "Nothing")
      {
    ...
    for(let i =0 ; i<shitcoins.length ; i++)
    {
      let sag  = await ichimokuIndicator(symbol = shitcoins[i] , interval = "1h")
      if(sag != "Nothing")
      {
    ...
    for(let i =0 ; i<shiiiiiitcoins.length ; i++)
    {
      let sag  = await ichimokuIndicator(symbol = shiiiiiitcoins[i] , interval = "1h")
      if(sag != "Nothing")
      {
